...
async def test_create_account(auth_client):
    resp = await auth_client.post("/api/accounts", json={"name": "测试账号", "phone": "13800138000"})
...
async def test_update_account(auth_client):
    ...
    acc_id = resp.json()["id"]
    resp = await auth_client.put(f"/api/accounts/{acc_id}", json={"name": "新名称", "phone": "13900001111"})
    assert resp.status_code == 200
